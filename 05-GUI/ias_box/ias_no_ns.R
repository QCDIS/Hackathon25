...
ias_app_ui <-
  ...
  tags$head(
    tags$style(HTML("
      ...
      .card { margin: 15px 0; }
      .card-header { background-color: #A0522D; color: white; }
      .card-body { padding: 15px; }
      .card { width: 100%; }
      .floating-logo {
        position: absolute; top: 0; right: 0; margin: 10px; height: 60px;
      }
  ...
  div(class = "header",
      h1("Invasive Alien Species Prototype Digital Twin")
  ),
    fluidRow(
      column(
        class = "my-3",
        ...
        card(
          ...
          downloadButton("downloadTif", "Download TIFF File")
        )
      ),
      column(
        8,
        ...
        card(
          ...
          full_screen = TRUE,
          card_header("pDT-IAS"),
          card_body(leafletOutput("rasterMap")),
          card_footer(class = "fs-6", uiOutput("selectedOptions"))
        )
      )
    )
  )
...
ias_app_server <- function(input, output, session) {
  ...
  output$timePeriodUI <- renderUI({
    if (input$timeFramePicker == "Present") {
      ...
      radioButtons("timePeriodPicker", "Time period:",
  ...
  output$climateModelUI <- renderUI({
    if (input$timeFramePicker == "Present") {
      ...
      pickerInput("climateModelPicker", "Climate model:",
  ...
  output$climateScenarioUI <- renderUI({
    if (input$timeFramePicker == "Present") {
      ...
      pickerInput("climateScenarioPicker", "Climate scenario:",
